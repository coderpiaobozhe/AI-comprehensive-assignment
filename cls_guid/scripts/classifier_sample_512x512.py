    ...
    def cond_fn(x, t, y=None, **kwargs):
        ...
        with th.no_grad():
            if args.use_entropy_scale:
                ...
                kl_scale = F.kl_div(log_probs, uniform, reduction='none').mean(dim=-1)
                kl_scale = kl_scale.reshape(-1, 1, 1, 1).repeat(1, *cond_grad[0].shape)
                guidance['scale'] = guidance['scale'] * kl_scale
